...
-- Step2:找出"資料表"與"資料表"之間的相關聯欄位
-- 1.LOCATIONS, 2.DEPARTMENTS, 3.EMPLOYEES, 4.JOBS

-- V 1.LOCATIONS ("LOCATION_ID")  DEPARTMENTS
-- V 2.DEPARTMENTS ("MANAGER_ID,EMPLOYEE_ID")  EMPLOYEES
-- 3.EMPLOYEES (JOB_ID) JOBS

-- Step3:運用Step1、Step2所找出的線索，寫SQL!
...
-- MySQL:CONCAT
-- MS SQL:CONCAT、+
-- Oracle:CONCAT(只能接受兩個引數)、||
SELECT CONCAT(STORE_ID, '_', STORE_NAME),
	CONCAT('$', SALES)
FROM store_information;

-- 123456
-- Boston
SELECT STORE_NAME, SUBSTRING(STORE_NAME, 2),
	SUBSTRING(STORE_NAME, 2, 3) 
FROM STORE_INFORMATION;

-- TRIM([[位置] [要移除的字串] FROM ] 字串)
SELECT STORE_ID, STORE_NAME,
	LTRIM(STORE_NAME), RTRIM(STORE_NAME), TRIM(STORE_NAME)
--    TRIM(0, 'Bos' FROM STORE_NAME)
FROM STORE_INFORMATION;
